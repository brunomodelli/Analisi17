...
public class Test {
	...
	public static float simulationAllLines(int year, boolean parsedLeagues)
		...
		try {
			...
			if (!parsedLeagues)
				file = new FileInputStream(new java.io.File(base + ALL_EURO_DATA + year + DASH + (year + 1) + XLS));
			else
				file = new FileInputStream(new java.io.File(base + "\\data\\fullodds" + year + XLS));
	...
	public static float asian(int year, boolean parsedLeagues)
		...
		try {
			...
			if (!parsedLeagues)
				...
				file = new FileInputStream(new java.io.File(base + "\\data\\odds" + year + XLS));
			HSSFWorkbook workbook = new HSSFWorkbook(file);
		} catch (Exception e) {
			System.out.println(WRONG);
		} finally {
	...
	public static float asianFinals(int year) throws IOException, InterruptedException, ExecutionException {
		...
		try {
			file = new FileInputStream(
					new java.io.File(base + ALL_EURO_DATA + year + DASH + (year + 1) + XLS));
		} catch (Exception e) {
			System.out.println(WRONG);
		} finally {
			if (file != null) {
	...
	public static final void aggregateInterval() throws IOException, InterruptedException, ExecutionException {
		...
		FileInputStream file;
		try {
			file = new FileInputStream(new java.io.File(base + ALL_EURO_DATA + 2014 + DASH + 2015 + XLS));
		} catch (Exception e) {
			System.out.println(WRONG);
		} finally {
			if (file != null) {
	...
	public static void makePredictions() throws IOException, InterruptedException, ParseException {
		...
		FileInputStream file;
		try {
			file = new FileInputStream(new java.io.File("fixtures.xls"));
			HSSFWorkbook workbook = new HSSFWorkbook(file);
		} catch (Exception e) {
			System.out.println(WRONG);
		} finally {
		...
		FileInputStream filedata;
		try {
			filedata = new FileInputStream(new java.io.File("all-euro-data-2015-2016.xls"));
			HSSFWorkbook workbookdata = new HSSFWorkbook(filedata);
		} catch (Exception e) {
			System.out.println(WRONG);
		} finally {
